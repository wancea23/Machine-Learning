...
x = torch.tensor(df[["TRIP_MILES", "TRIP_SECONDS", "TRIP_SPEED"]].values, dtype=torch.float32, device=device)
...
w = torch.tensor([0.0, 0.0, 0.0], device=device, requires_grad=True)
...
print("-" * 60)
print(f"Final model: FARE = {w_list[0]:.4f} * TRIP_MILES + {w_list[1]:.4f} * TRIP_SECONDS + {w_list[2]:.4f} * TRIP_SPEED + {b.item():.4f}")
...
with torch.no_grad():
    ...
    for idx in random_indices:
        ...
        print(f"Features: Miles={features[0]:.2f}, Sec={features[1]:.0f}, Speed={features[2]:.1f}")
